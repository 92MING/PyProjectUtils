...
def getSubMask(ip: str=None):
    '''获取子网掩码（默认本地ip的子网掩码）'''
    ip_address = getLocalIP() if ip is None else ip
    # 遍历所有网卡获取子网掩码
...
import stun
from source.PyProjectUtils.base_class.CrossModuleEnum import CrossModuleEnum
class NATtype(CrossModuleEnum):
    Unknown = 0
    Blocked = 1
    OpenInternet = 2
    FullCone = 3
